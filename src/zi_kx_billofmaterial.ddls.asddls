...
define view entity ZI_KX_BillOfMaterial
  as select from    ZI_MatBOMLink           as _BOMHeader
    inner join      ZI_BOMItem              as _BOMItem  on  _BOMHeader.BillOfMaterial = _BOMItem.BillOfMaterial
                                                         and _BOMHeader.AlternativeBOM = _BOMItem.AlternativeBOM
    inner join      ZI_MaterialData         as _Material on _Material.MaterialNumber = _BOMItem.BOMComponent
    left outer join ZI_KX_ProdUoM           as _ProdUoM  on  _ProdUoM.MaterialNumber           = _BOMItem.BOMComponent
                                                         and _ProdUoM.Plant                    = _BOMHeader.Plant
                                                         and _ProdUoM.AlternativeUnitOfMeasure = _BOMItem.ComponentUnitOfMeasure
    left outer join ZI_BOMProductionVersion as _ProdVer  on  _BOMHeader.MaterialNumber = _ProdVer.MaterialNumber
